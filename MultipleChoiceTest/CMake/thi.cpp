// thi.cpp
// - Thi trac nghiem co luu bai dang lam (tiep tuc khi bi vang)
// - Thanh trang thai dap an + danh dau cau hoi (Mark)
// - Canh bao thi lai, ghi de diem cu
...
// Do phuc tap thoi gian: O(Tong so SV)
...
// xoa 1 dong tren console (toi da WIDTH ky tu)
static void clear_line_at(int row) {
    const int WIDTH = 120; // du lon so voi console
    gotoxy(0, row);
    for (int i = 0; i < WIDTH; ++i) putchar(' ');
    gotoxy(0, row);
}

// ve thanh trang thai dap an (so cau + dap an + danh dau + mui ^ chi cau hien tai)
...
// Dong 2..: id da_chon markFlag(0/1)
//
...
static bool save_inprogress_exam(const char* masv_ci, const char* mamh_ci,
                                 const ExamQItem* items, const bool* marked,
...
// Do phuc tap: O(soCau * soCau_trong_mon) trong truong hop xau nhat
                                 ...
                                 int& soCau, int& curIdx, int& remainSec)
{
    ...
    int* mk   = new int[soCau];

    for (int i = 0; i < soCau; ++i) {
        int id; char c; int m;
        if (std::fscanf(f, "%d %c %d", &id, &c, &m) != 3) {
            delete[] ids;
            delete[] ans;
            delete[] mk;
            std::fclose(f);
...
//
// Logic:
// - Neu ton tai file InProgress_<MASV>_<MAMH>.txt:
//     + Hien thong tin -> hoi "co tiep tuc khong?"
//     + YES: tiep tuc dung de cu (khong hoi so cau / so phut).
//     + NO : xoa file tam, hoi lai so cau / so phut -> tao de moi.
// - Trong khi thi, moi lan co thay doi (chuyen cau, chon dap an, danh dau)
//   se luu lai trang thai moi nhat.
// - Khi nop bai / het gio: xoa file tam, cham diem, cap nhat ExamLog + diem SV.
//
                  ...
                  int soCau, int thoiGianPhut)
{
    ...
    if (!hasSaved || !items) {
        ...
        save_inprogress_exam(masv_ci, mamh_ci, items, mark, soCauExam, curIdx, remainSec);
    }
    ...
    printf("THI TRAC NGHIEM - SV: %s  - Lop: %s  - Mon: %s\n",
           ...
           lop ? lop->malop : "(?)",
           mh->data.mamh);
    printf("So cau: %d   (M: danh dau/bo danh dau, F9: NOP SOM, ESC: HOI NOP BAI)\n", soCauExam);
    printf("---------------------------------------------------------------\n");
    ...
    while (1) {
        ...
        // Xoa dong trang thai truoc khi in (de khong bi du chu)
        clear_line_at(ROW_QUESTION + 6);

        const char* markText = mark[cur] ? "CO" : "KHONG";
        if (items[cur].da_chon == '?')
            printf("Da chon: (chua chon)   Trang thai danh dau: %-6s\n", markText);
        else
            printf("Da chon: %c           Trang thai danh dau: %-6s\n",
                   items[cur].da_chon, markText);

        // Thanh trang thai dap an
        draw_answer_bar(items, mark, soCauExam, cur);
        ...
        if (key == KEY_ESC) {
            if (confirm_dialog("Ban co muon NOP BAI ngay bay gio khong")) {
                ...
                save_inprogress_exam(masv_ci, mamh_ci, items, mark, soCauExam, cur, remain);
                break;
            }
        } else if (key == KEY_F9) {
